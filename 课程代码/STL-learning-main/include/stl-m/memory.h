#ifndef DEALII_MEMORY_H_
#define DEALII_MEMORY_H_
...
namespace mystl
{
  ...
  private:
    ...
    allocate_buffer();
    void
    initialize_buffer(const T &, std::true_type)
    {}
    void
    initialize_buffer(const T &value, std::false_type)
    {
  ...
  template <class ForwardIterator, class T>
  void
  temporary_buffer<ForwardIterator, T>::allocate_buffer()
  {
    original_len = len;
    if (len > static_cast<ptrdiff_t>(INT_MAX / sizeof(T)))
      len = INT_MAX / sizeof(T);
