#ifndef DEALII_VECTOR_H_
#define DEALII_VECTOR_H_
...
#  undef min
  ...
  template <class T>
  void
  vector<T>::resize(size_type new_size, const value_type &value)
  {
    if (new_size < size())
      {
  ...
  template <class IIter>
  void
  vector<T>::copy_assign(IIter first, IIter last, input_iterator_tag)
  {
    auto cur = begin_;
  ...
  template <class FIter>
  void
  vector<T>::copy_assign(FIter first, FIter last, forward_iterator_tag)
  {
  ...
  template <class IIter>
  void
  vector<T>::copy_insert(iterator pos, IIter first, IIter last)
  {
    if (first == last)
      return;
  ...
  template <class T>
  void
  vector<T>::reinsert(size_type size)
  {
