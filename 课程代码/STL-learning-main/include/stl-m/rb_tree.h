#ifndef DEALII_RB_TREE_H_
#define DEALII_RB_TREE_H_
...
namespace mystl
{
    ...
    : public mystl::iterator<mystl::bidirectional_iterator_tag, T>
  {
    ...
    bool
    operator==(const rb_tree_iterator_base &rhs)
    {
  ...
  NodePtr
  rb_tree_min(NodePtr x) noexcept
  {
  ...
  template <class NodePtr>
  NodePtr
  rb_tree_max(NodePtr x) noexcept
  {
  ...
  template <class NodePtr>
  bool
  rb_tree_is_lchild(NodePtr node) noexcept
  {
  ...
  template <class NodePtr>
  bool
  rb_tree_is_red(NodePtr node) noexcept
  {
  ...
  template <class NodePtr>
  void
  rb_tree_set_black(NodePtr &node) noexcept
  {
  ...
  template <class NodePtr>
  void
  rb_tree_set_red(NodePtr &node) noexcept
  {
  ...
  template <class NodePtr>
  NodePtr
  rb_tree_next(NodePtr node) noexcept
  {
    if (node->right != nullptr)
  ...
  template <class NodePtr>
  void
  rb_tree_rotate_left(NodePtr x, NodePtr &root) noexcept
  {
  ...
  template <class NodePtr>
  void
  rb_tree_rotate_right(NodePtr x, NodePtr &root) noexcept
  {
    auto y  = x->left;
    x->left = y->right;
    if (y->right)
  ...
  template <class NodePtr>
  void
  rb_tree_insert_rebalance(NodePtr x, NodePtr &root) noexcept
  {
  ...
  template <class NodePtr>
  NodePtr
  rb_tree_erase_rebalance(NodePtr  z,
                          NodePtr &root,
                          NodePtr &leftmost,
                          NodePtr &rightmost)
  {
  ...
  public:
    ...
    iterator
    insert_multi(value_type &&value)
    {
      return emplace_multi(mystl::move(value));
    }

    iterator
    insert_multi(iterator hint, const value_type &value)
    {
      return emplace_multi_use_hint(hint, value);
    }
    iterator
    insert_multi(iterator hint, value_type &&value)
    {
    ...
    iterator
    insert_unique(iterator hint, value_type &&value)
    {
    ...
    size_type
    count_multi(const key_type &key) const
    {
  ...
  private:
    ...
    base_ptr
    copy_from(base_ptr x, base_ptr p);
    void
  ...
  rb_tree<T, Compare>::emplace_multi_use_hint(iterator hint, Args &&...args)
  {
    ...
    if (hint == begin())
      ...
      { // 位于 end 处
        if (!key_comp_(
              key, value_traits::get_key(rightmost()->get_node_ptr()->value)))
          {
  ...
  { // 返回一个 pair，第一个值为一个 pair，包含插入点的父节点和一个 bool
    // 表示是否在左边插入，
  ...
  template <class T, class Compare>
  typename rb_tree<T, Compare>::iterator
  rb_tree<T, Compare>::insert_value_at(base_ptr          x,
  ...
  template <class T, class Compare>
  typename rb_tree<T, Compare>::iterator
  rb_tree<T, Compare>::insert_node_at(base_ptr x,
                                      node_ptr node,
                                      bool     add_to_left)
  {
    node->parent   = x;
  ...
  template <class T, class Compare>
  typename rb_tree<T, Compare>::iterator
  rb_tree<T, Compare>::insert_multi_use_hint(iterator hint,
                                             key_type key,
                                             node_ptr node)
  {
    // 在 hint 附近寻找可插入的位置
  ...
  template <class T, class Compare>
  typename rb_tree<T, Compare>::iterator
  rb_tree<T, Compare>::insert_unique_use_hint(iterator hint,
                                              key_type key,
                                              node_ptr node)
  {
    // 在 hint 附近寻找可插入的位置
  ...
  template <class T, class Compare>
  bool
  operator==(const rb_tree<T, Compare> &lhs, const rb_tree<T, Compare> &rhs)
  {
  ...
  template <class T, class Compare>
  void
  swap(rb_tree<T, Compare> &lhs, rb_tree<T, Compare> &rhs) noexcept
  {
    lhs.swap(rhs);
  }

} // namespace mystl
#endif // !DEALII_RB_TREE_H_
