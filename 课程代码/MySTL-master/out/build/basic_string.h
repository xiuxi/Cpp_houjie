...
template <class CharType>struct char_traits {		//char基类
	typedef CharType char_type;

	static size_t length(const char_type* str) {		//计算字符串长度
		size_t len = 0;
	...
	static char_type* copy(char_type* dst, const char_type* src, size_t n) {	//拷贝字符串
	...
	static char_type* move(char_type* dst, const char_type* src, size_t n) {	//移动字符串
		char_type* r = dst;
		if (dst < src) {
			for (; n != 0; --n, ++dst, ++src)
				*dst = *src;
	...
	static char_type* fill(char_type* dst, char_type* ch, size_t count) {		//填充字符串
		char_type* r = dst;
...
template <>struct char_trais<char> {	//偏特化模板类
	typedef char char_type;

	//以下创建接口去调用C语言的函数
	static size_t length(const char_type* str)noexcept {	//计算字符串长度
		return std::strlen(str);
	}

	static int compare(const char_type* s1, const char_type* s2, size_t n)noexcept {	//比较字符串
		return std::memcmp(s1, s2, n);
	}

	static char_type* copy(char_type* dst, const char_type* src, size_t n) noexcept {	//拷贝字符串
		MYSTL_DEBUG(src + n <= dst || dst + n <= src);
		return static_cast<char_type*>(std::memcpy(dst, src, n));
	}

	static char_type* move(char_type* dst, const char_type* src, size_t n) noexcept		//移动字符串
	{
		return static_cast<char_type*>(std::memmove(dst, src, n));
	}

	static char_type* fill(char_type* dst, char_type ch, size_t count) noexcept			//填充字符串
	{
		return static_cast<char_type*>(std::memset(dst, ch, count));
	}
};

// Partialized. char_traits<wchar_t>	宽字节的char
template <>
struct char_traits<wchar_t> {			//模板偏特化类
...
// Partialized. char_traits<char16_t> 16位的char
template <>
...
// Partialized. char_traits<char32_t> 32位的char
template <>
...
public:
	...
	//静态断言
	static_assert(std::is_pod<CharType>::value, "Character type of basic_string must be a POD");	//判断是否POD型别
	static_assert(std::is_same<CharType, typename traits_type::char_type>::value,			//判断型别是否相同
		"CharType must be same as traits_type::char_type");

public:
	// 末尾位置的值，例:
	// if (str.find('a') != string::npos) { /* do something */ }	
	static constexpr size_type npos = static_cast<size_type>(-1);//这是一个无符号的类型，是string容易可容纳的最大大小
...
public:
	...
	basic_string() noexcept			//构造函数
	{
		try_init();
	}

	basic_string(size_type n, value_type ch)		//构造函数，使用初始化列表
		:buffer_(nullptr), size_(0), cap_(0)
	{
		fill_init(n, ch);
	}

	basic_string(const basic_string& other, size_type pos)		//构造函数
		:buffer_(nullptr), size_(0), cap_(0)
	{
		init_from(other.buffer_, pos, other.size_ - pos);
	}
	basic_string(const basic_string& other, size_type pos, size_type count)		//构造函数
		:buffer_(nullptr), size_(0), cap_(0)
	{
		init_from(other.buffer_, pos, count);
	}

	basic_string(const_pointer str)							//构造函数
		:buffer_(nullptr), size_(0), cap_(0)
	{
		init_from(str, 0, char_traits::length(str));
	}
	basic_string(const_pointer str, size_type count)		//构造函数
		:buffer_(nullptr), size_(0), cap_(0)
	{
	...
	template <class Iter, typename std::enable_if<
		...
		basic_string(Iter first, Iter last)							//构造函数模板
	{
		copy_init(first, last, iterator_category(first));
	}

	basic_string(const basic_string& rhs)							//拷贝构造函数
		:buffer_(nullptr), size_(0), cap_(0)
	{
		init_from(rhs.buffer_, 0, rhs.size_);
	}
	basic_string(basic_string&& rhs) noexcept						//移动构造函数
	...
	basic_string& operator=(const basic_string& rhs);				//拷贝赋值运算符重载，左值版本
	basic_string& operator=(basic_string&& rhs) noexcept;			//拷贝赋值运算符重载，右值版本

	basic_string& operator=(const_pointer str);						//拷贝赋值运算符
	basic_string& operator=(value_type ch);							//拷贝赋值运算符

	~basic_string() { destroy_buffer(); }							//析构函数
...
operator=(const_pointer str)
{
	...
	if (cap_ < len)
	{
		auto new_buffer = data_allocator::allocate(len + 1);	//配置内存
		data_allocator::deallocate(buffer_);
		buffer_ = new_buffer;									//字符串赋值
		cap_ = len + 1;
	}
