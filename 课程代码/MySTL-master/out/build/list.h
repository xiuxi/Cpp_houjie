...
//list的结点结构
template <class T> struct list_node_base {		//父类提供前一节点与后一节点指针
...
template<class T>struct list_node :public list_node_base<T> {		//子类继承父类，并提供value
	typedef typename node_traits<T>::base_ptr base_ptr;
	typedef typename node_traits<T>::node_ptr node_ptr;

	T value;	// 数据域
	list_node() = default;		//使用默认构造函数
	list_node(const T& v):value(v){}	//拷贝构造函数
	list_node(T&& v):value(mystl::move(v)){}	//移动构造函数
	base_ptr as_base()
	{
		return static_cast<base_ptr>(&*this);	//&*是指针的引用
	}
	node_ptr self()
	{
...
// list 的迭代器设计,提供Bidirectional iterator
template <class T> struct list_iterator :public mystl::iterator<mystl::bidirectional_iterator_tag, T> {
	//内嵌型别定义
	...
	//重载操作符
	reference operator*()const { return node->as_node()->value; }
	pointer operator->()const { return &(operator*()); }

	self& operator++() {	//前置++，返回reference，因为C++允许前置++连续做两次，如++++i
		MYSTL_DEBUG(node_ != nullptr);
		node_ = node_->next;
		return *this;
	}	
	self operator++(int) {//后置++，(int)只是为了区分前置++与后置++，实际int没意义，返回value,不允许后置++连续做两次
		self tmp = *this;	//记录原值,不会调用operator*,因为编译器先遇到operator=，知道是个赋值动作
		++* this;			//当前节点+1
		return tmp;			//返回原值
	}
	self& operator--() {
...
//list的const迭代器设计
template<class T>struct list_const_iterator : public iterator<bidirectional_iterator_tag, T> {
	typedef T									value_type;
...
public:
	...
	// emplace_front / emplace_back / emplace		//相比vector，多了emplace_front
	template<class...Args>void emplace_front(Args&& ...args){
		THROW_LENGTH_ERROR_IF(size_ > max_size() - 1, "list<T>'s size too big");	//要求容量超出范围抛出异常
		auto link_node = create_node(mystl::forward<Args>(args)...);
		link_nodes_at_front(link_node->as_base(), link_node->base());
		++size_;
	}
	...
	{
		THROW_LENGTH_ERROR_IF(size_ > max_size() - 1, "list<T>'s size too big");
		auto link_node = create_node(value);		//左值版本
		++size_;
	...
	{
		THROW_LENGTH_ERROR_IF(size_ > max_size() - 1, "list<T>'s size too big");
		auto link_node = create_node(mystl::move(value));	//右值版本
		++size_;
		return link_iter_node(pos, link_node->as_base());
	}
	...
	//unique操作，移除数值相同的连续元素
	//只有“连续且相同的元素”，才会被移除剩一个
	void unique()
	{
	...
	//merge归并操作，将x合并到*this身上，两个list内容必须先经过递增排序
	void merge(list& x)
	{
	...
	//sort排序操作，STL的sort()只接受RamdonAccessIterator，所以必须自己定义一个sort()
	void sort()
	{
...
private:
	...
	template <class Iter>iterator  copy_insert(const_iterator pos, size_type n, Iter first);

	// sort
	template <class Compared>iterator  list_sort(iterator first, iterator last, size_type n, Compared comp);
...
// 删除 pos 处的元素
template <class T>typename list<T>::iterator list<T>::erase(const_iterator pos) {
	MYSTL_DEBUG(pos != cend());
	auto n = pos.node_;
	auto next = n->next;
	ulink_nodes(n, n);			//将容器与[first,last]断开连接
...
// 删除 [first, last) 内的元素
template <class T>typename list<T>::iterator list<T>::erase(const_iterator first, const_iterator last) {
	if (first != last) {
		unlink_nodes(first.node_, last.node_->prev);
		while (first != last) {		//遍历[first，last]，将区间内的元素逐一销毁
...
template <class T>void list<T>::clear() {
	if (size_ != 0) {
		auto cur = node_->next;
		for (base_ptr next = cur->nex; cur != node_; cur = nex, next = cur->next) {	//遍历容器进行元素销毁
			destroy_node(cur->as_node());
		}
		node_->unlink();	//断开连接
		size = 0;
...
// 重置容器大小
template <class T>void list<T>::resize(size_type new_size, const value_type& value) {
	auto i = begin();
	size_type len = 0;
	while (i != end() && len < new_size) {	//遍历容器
		++i;
		++len;
	}
	if (len == new_size) {					//如果长度等于新的大小，删除多余元素
		erase(i, node);
	}
	else {									//否则插入新元素
...
template <class T> void list<T>::splice(const_iterator pos, list& x) {
	...
	if (!x.empty()) {
		...
		x.unlink_nodes(f, l);			//容器与[first, last] 结点断开连接
		link_nodes(pos.node_, f, l);	//在 pos 处连接 [first, last] 的结点
...
template <calss T>void list<T>::splice(const_iterator pos, list& x, const_iterator it) {
	if (pos.node_ != it.node_ && pos.node_ != it.node_->next) {
		THROW_LENGTH_ERROR_IF(size_ > max_size() - 1, "list<T>'s size too big");
		auto f = it.node_;
		x.unlink_nodes(f, f);			//断开list x的it所指节点连接
		link_nodes(pos.node_, f, f);	//将pos所指节点结合于it所指节点之前
		++size_;						//当前list容量+1
		--x.size_;						//list x大小-1
	}
}
...
template <class T>void list<T>::splice(const_iterator pos, list& x, const_iterator first, const_iterator last){
	if (first != last && this != &x)
	{
		...
		x.unlink_nodes(f, l);			//断开list x的[first,last]
		link_nodes(pos.node_, f, l);	//接合于pos之前

		size_ += n;						//当前list容量+n
		x.size_ -= n;					//list x大小-n
	}
}

// 将另一元操作 pred 为 true 的所有元素移除
template <class T>template <class UnaryPredicate>void list<T>::remove_if(UnaryPredicate pred) {
	auto f = begin();
	auto l = end();
	for (auto next = f; f != 1; f = next) {
		++next;
		if (pred(*f)) {		//判断为true就移除该元素
			erase(f);
		}
	}
}
...
template <class T>template <class BinaryPredicate>void list<T>::unique(BinaryPredicate pred) {
	auto i = begin();
	auto e = end();
	auto j = i;
	++j;
	while (j != e) {
		if (pred(*i, *j)) {		//判断list中元素是否为ture，是就移除
			erase(j);
		}
		else {
			i = j;
...
template <class T>template <class Compare>void list<T>::merge(list& x, Compare comp) {
	if (this != &x) {
		THROW_LENGTH_ERROR_IF(size_ > max_size() - x.size_, "list<T>'s size too big");
		auto f1 = begin();
		auto l1 = end();
...
template <class T>void list<T>::reverse() {
	...
	{
		mystl::swap(i.node_->prev, i.node_->next);	//每次交换当前节点的前一节点与后一节点
		i.node_ = i.node_->prev;					//从前往后
	}
	mystl::swap(e.node_->prev, e.node_->next);		//交换最后一个节点的前一节点与后一节点
}
...
{
	node_ptr p = node_allocator::allocate(1);		//创建内存
	try
	{
		data_allocator::construct(mystl::address_of(p->value), mystl::forward<Args>(args)...);	//构造对象
		p->prev = nullptr;
		p->next = nullptr;
	}
	catch (...)
	{
		node_allocator::deallocate(p);			//有异常就释放刚创建的空间
		throw;
	}
	return p;
}
...
{
	data_allocator::destroy(mystl::address_of(p->value));	//析构对象
...
void list<T>::fill_init(size_type n, const value_type& value)
{
	...
	try
	{
		for (; n > 0; --n)
		{
			auto node = create_node(value);	//创建节点node
			link_nodes_at_back(node->as_base(), node->as_base());	//构造父类对象
		}
	}
	catch (...)		//异常就进行元素清除与释放内存
	{
		clear();
		base_allocator::deallocate(node_);	
		node_ = nullptr;
...
{
	if (pos == node_->next)		//pos为node的下一节点
	{
		link_nodes_at_front(link_node, link_node);	//将list插入到pos之前
	}
	else if (pos == node_)		//pos为node
	{
		link_nodes_at_back(link_node, link_node);	//将list接合到pos之后
	}
	else
	{
		link_nodes(pos.node_, link_node, link_node);	//接合list
	}
	return iterator(link_node);
}
...
void list<T>::copy_assign(Iter f2, Iter l2)
{
	...
	{
		insert(l1, f2, l2);		//将[f2,l2)的值插入到list1
	}
}
