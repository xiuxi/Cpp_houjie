...
public:
	// map 的嵌套型别定义
	typedef Key                        key_type;		//map的key值型别
	typedef T                          mapped_type;		//map的value型别
	typedef mystl::pair<const Key, T>  value_type;      //数据(实值)型别，即pair<const Key,T>,表示具体值的型别
    ...
    public:
        ...
        {
            return comp(lhs.first, rhs.first);  //调用"元素比较函数"，比较键值的大小
        }
    };

private:
    // 以 mystl::rb_tree 作为底层机制
    //以map元素型别(一个pair)的第一型别，作为红黑树节点的键值型别
    typedef mystl::rb_tree<value_type, key_compare>  base_type;
    base_type tree_;    //以红黑树表现map
...
public:
    ...
    //iterator不为const，不允许改变键值，但可以通过迭代器改变元素的实值(value)
...
public:
    ...
    //map不允许相同键值存在，调用红黑树的insert_unique
